...
class SakaCalendar:
    ...
    SAKA_MONTH_NAMES= ["Chaitra", "Vaisakha", "Jyaishta", "Asadha", "Sravana",
                       "Bhadra", "Aswina", "Kartiak", "Agrahayana", "Pausa",
                       "Magha", "Phalguna"]
    SAKA_WEEK_NAMES= ["Ravivar", "Somavar", "Mangalvar", "Budhvar",
                      "Sukhravar", "Guruvar", "Sanivar"]
    IE = 0
    ...
    def get_month_length(self, extendedYear, month):
        if month < 0 or month > 11:
            ...
            month = month % 12
        if (self.is_gregorian_leap(extendedYear + self.INDIAN_ERA_START)
        and month == 0):
            return 31
...
This routine converts a gregorian date to the corresponding Julian date"
    ...
    def gregorian_to_julian_date(self, year, month, date):
        jd = ((self. JULIAN_EPOCH - 1) +
              ...
              math.floor((((367 * month) - 362) / 12) +
                         (0 if (month <= 2) else
                          -1 if self.is_gregorian_leap(year) else -2)) +
              date)
        return jd
    ...
    def julian_date_to_gregorian(self, jd):
        ...
        leapadj = (0 if (wjd < self.gregorian_to_julian_date(year, 3, 1))
                   else 1 if self.is_gregorian_leap(year) else 2)
    ...
    def gregorian_to_saka_date(self, gregorianDay):
        ...
        IndianYear = gregorianDay[0] - self.INDIAN_ERA_START
        # Year in Saka era
        jdAtStartOfGregYear = self.gregorian_to_julian_date(
            gregorianDay[0], 1, 1)  # JD at start of Gregorian year
        julianDay = self.gregorian_to_julian_date(gregorianDay[0],
                                                  gregorianDay[1],
                                                  gregorianDay[2])
        yday = int(julianDay - jdAtStartOfGregYear)
              # Day number in Gregorian year (starting from 0)
        if yday < self.INDIAN_YEAR_START:
            #  Day is at the end of the preceding Saka year
            IndianYear -= 1
            leapMonth = 31 if self.is_gregorian_leap(gregorianDay[0]
                                                     - 1) else 30
            # Days in leapMonth this year, previous Gregorian year
            yday += leapMonth + (31 * 5) + (30 * 3) + 10
        else:
            leapMonth = 31 if self.is_gregorian_leap(gregorianDay[0]) else 30
        ...
        if yday < leapMonth:
            ...
            if mday < (31 * 5):
                ...
                mday -= 31 * 5
                IndianMonth = int(math.floor(mday /
                                             30)) + 6
