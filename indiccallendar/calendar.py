...
class Calendar:
    ...
    def date(self, calendar_system="Gregorian"):
        ...
        if calendar_system == "Saka":
            ...
            greg = [datetime.date.today().year,
                    datetime.date.today().month,
                    datetime.date.today().day]
            return sakacalendar.gregorian_to_saka_date(greg)

    def convert(self, from_calendar_system,
                to_calendar_system,
                year, month,
                day):
        ...
        if to_calendar_system   == "Gregorian":
            return sakacalendar.julian_date_to_gregorian(jd)
        if to_calendar_system == "Saka":
            return sakacalendar.gregorian_to_saka_date(sakacalendar.julian_date_to_gregorian(jd))
